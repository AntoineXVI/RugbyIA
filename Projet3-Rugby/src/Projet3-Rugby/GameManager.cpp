...
void GameManager::Update()
{
	...
	for (int i = 0; i < mEntities.size(); i++)
	{
		...
		if (Utils::isCollide(mEntities[i], mBall) && mEntities[i]->GetState() != Context::State::GetBall && mEntities[i]->GetState() != Context::State::Idle && mEntities[i]->GetState() != Context::State::EnemyNear && !mEntities[i]->isInvicible())
		{
